<?php
    ...
    class UserHandler {
            ...
            try {
                ...
                if ($patients) {
                    ...
                    foreach ($patients as &$patient) {
                        ...
                        $patient['gender'] = $patient['gender'] ?? 'Not provided';
                        $patient['date_of_birth'] = $patient['date_of_birth'] ?? 'Not provided';
                        $patient['home_address'] = $patient['home_address'] ?? 'Not provided';
                        $patient['contact_number'] = $patient['contact_number'] ?? 'Not provided';
                        $patient['height'] = $patient['height'] ?? 0;
                        $patient['weight'] = $patient['weight'] ?? 0;
                        $patient['medications'] = $patient['medications'] ?? 'Not provided';
                        $patient['birthplace'] = $patient['birthplace'] ?? 'Not provided';
                        $patient['nationality'] = $patient['nationality'] ?? 'Not provided';
                        $patient['religion'] = $patient['religion'] ?? 'Not provided';
                        $patient['civil_status'] = $patient['civil_status'] ?? 'Not provided';
                        $patient['age'] = $patient['age'] ?? 'Not provided';
                    }
        ...
        public function getUsers() {
            $query = "SELECT u.id, u.firstname, u.lastname, u.gender, u.email, 
                             u.contact_number, u.home_address, u.birthplace, u.age, 
                             u.nationality, u.religion, u.civil_status,
                             p.patient_id  -- Add this line to get patient_id
                      FROM users u
                      LEFT JOIN patients p ON u.id = p.id";  // Join with patients table
        ...
        public function getAppointments($patientId) {
            ...
            try {
                ...
                foreach ($appointments as &$appointment) {
                    $date = new DateTime($appointment['appointment_date'], new DateTimeZone('UTC'));
    ...
    public function scheduleAppointment($data) {
        try {
            ...
            $appointmentDate = new DateTime($schedule['available_date'], new DateTimeZone('Asia/Manila'));
            $appointmentDate->setTimezone(new DateTimeZone('UTC'));
            $formattedDate = $appointmentDate->format('Y-m-d');
    
            // Update schedule status to booked
            $updateQuery = "UPDATE doctor_schedules 
                           SET status = 'booked', 
                               updated_at = NOW() 
                           WHERE schedule_id = ?";
            ...
            // Create the appointment with UTC date
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        if (isset($_GET['action'])) {
            ...
            elseif ($action === 'getUsers') {
                ...
                echo json_encode($response);
             } elseif ($action === 'getPatientAppointments') {
                    $response = $userHandler->getPatientAppointments();
                    echo json_encode($response);
    }
